...
if (!baseUrl || !executablePath || !outputDir) {
  throw new Error("Usage: check-eni-price-cap-browser.cjs BASE_URL BROWSER_EXE OUTPUT_DIR");
}
...
async function inspect(browser, name, viewport, url, screenshot) {
  ...
  await page.evaluate(() => {
    for (const image of [...document.images].filter(image => !/^https?:\/\//.test(image.getAttribute("src") || ""))) {
      image.loading = "eager";
    }
  });
  ...
  const result = await page.evaluate(() => ({
    ...
    images: [...document.images].filter(image => !/^https?:\/\//.test(image.getAttribute("src") || "")).map(image => ({
  ...
  for (const image of result.images) {
    if (!image.complete || image.naturalWidth === 0 || image.renderedWidth <= 0) throw new Error(`${name}: broken image ${JSON.stringify(image)}`);
  }
...
(async () => {
  ...
  try {
    const article = "/article-economia-energia-eni-price-cap-carburanti.html";
    const results = [];
