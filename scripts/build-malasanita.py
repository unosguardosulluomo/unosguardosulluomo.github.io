"""Build the Malasanità dossier and its editorial placements from the approved DOCX."""
...
def testimony(document):
    text = document.tables[1].cell(0, 0).text
    paragraphs = [line.strip() for line in text.splitlines() if line.strip()]
    return '<blockquote class="article-testimony">' + "".join(
        f"<p>{esc(line)}</p>" for line in paragraphs
    ) + "</blockquote>"


def data_table(document):
    rows = document.tables[2].rows
    ...
    return (
        ...
        '<div class="data-table-wrap" role="region" aria-label="Prestazioni specialistiche per tipo di erogatore" tabindex="0">'
        '<table class="data-table"><caption>Specialistica per erogatore in Lombardia, 2022-2025</caption>'
...
PULLQUOTES = (
    "La ricetta scade.",
    "Quando il Servizio Sanitario non dà una risposta",
    "Il Covid non ha creato il privato.",
    "Non sempre manca la prestazione.",
    "La lista d’attesa non sposta soltanto",
    "Il cittadino paga da solo quando non vuole aspettare.",
    "Perché continuare a comprare capacità privata",
)
...
for paragraph in document.paragraphs[5:]:
    ...
    if style == "Heading 1":
        if in_bullets:
            body.append("</ul>")
            in_bullets = False
        if text == "IL CANALE AGEVOLATO: QUANDO IL PRIVATO DIVENTA QUASI UN TICKET":
            body.append(image_figure())
        heading_number += 1
        anchor = f"sezione-{heading_number}"
        contents.append(f'<li><a href="#{anchor}">{esc(text)}</a></li>')
        body.append(f'<h2 id="{anchor}">{esc(text)}</h2>')
        continue
    if text.startswith(("Il bisogno prescritto nel pubblico", "Il grande ospedale pubblico che invecchia")):
        continue
    if text.startswith(("•", "", "�")):
        if not in_bullets:
            body.append('<ul class="article-list">')
            in_bullets = True
        body.append(f"<li>{esc(text[1:].strip())}</li>")
        continue
    if in_bullets:
        body.append("</ul>")
        in_bullets = False
    if text.startswith("RASETTI PAOLO"):
        body.append(f'<p class="testimony-label">{rich_paragraph(paragraph)}</p>')
        body.append(testimony(document))
    elif text.startswith(PULLQUOTES):
        body.append(f'<blockquote class="article-pullquote">{esc(text)}</blockquote>')
    else:
        body.append(f"<p>{rich_paragraph(paragraph)}</p>")
    if text.startswith("Il dataset ufficiale di Regione Lombardia"):
        body.append(data_table(document))
...
def dated_cards(markup):
    ...
    return sorted(
        cards,
        key=lambda item: re.search(r'datetime="(\d{4}-\d{2}-\d{2})"', item).group(1),
        reverse=True,
    )
...
archive = re.sub(
    r'<article class="archive-card" data-dossier="' + re.escape(D["slug"]) + r'".*?</article>',
    "",
    archive,
    flags=re.S,
)
...
indagini, count = section.subn(
    lambda match: match.group(1) + "".join(category_cards[:3]) + match.group(2),
    indagini,
    count=1,
)
assert count == 1, "Società selection not found"
indagini = re.sub(
    r'<time class="current-date" datetime="[^"]+">.*?</time>',
    f'<time class="current-date" datetime="{D["datePublished"]}">{D["dateLabel"]}</time>',
    indagini,
...
all_archive_cards = []
for category_archive in (
    "archivio-politica-italiana.html",
    "archivio-politica-internazionale.html",
    "archivio-economia.html",
    "archivio-societa.html",
):
    all_archive_cards.extend(dated_cards((ROOT / category_archive).read_text(encoding="utf-8")))
all_archive_cards = sorted(
    all_archive_cards,
    key=lambda item: re.search(r'datetime="(\d{4}-\d{2}-\d{2})"', item).group(1),
    reverse=True,
)
...
home, count = re.subn(
    r'(<section class="story-grid" aria-label="Ultime indagini">).*?(</section>)',
    lambda match: match.group(1) + latest_cards + match.group(2),
    home,
    count=1,
    flags=re.S,
)
...
home, count = re.subn(
    r'(<section class="story-grid" aria-label="Altre indagini recenti">).*?(</section>)',
    lambda match: match.group(1) + other_cards + match.group(2),
    home,
    count=1,
    flags=re.S,
)
...
print("Built Malasanità dossier, placements and sitemaps")
