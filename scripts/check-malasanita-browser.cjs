...
if (!baseUrl || !executablePath || !outputDir) {
  throw new Error("Usage: check-malasanita-browser.cjs BASE_URL BROWSER_EXE OUTPUT_DIR");
}
...
async function inspect(browser, name, viewport, url, screenshot) {
  ...
  await page.evaluate(() => {
    for (const image of [...document.images].filter(image => /(?:malasanita|testata)/.test(image.getAttribute("src") || ""))) {
      image.loading = "eager";
    }
  });
  ...
  await page.waitForFunction(() => [...document.images]
    ...
    .filter(image => /(?:malasanita|testata)/.test(image.getAttribute("src") || ""))
  ...
  for (const image of result.images) {
    ...
    if (!/(?:malasanita|testata)/.test(image.src || "")) continue;
...
(async () => {
  ...
  try {
    const results = [];
    results.push(await inspect(browser, "article-desktop", { width: 1440, height: 900 }, "/article-malasanita-liste-attesa-lombardia.html", "article-desktop-full.png"));
    results.push(await inspect(browser, "article-mobile", { width: 390, height: 844 }, "/article-malasanita-liste-attesa-lombardia.html", "article-mobile-full.png"));
